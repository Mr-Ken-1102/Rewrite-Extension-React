export const RWA_POPUP_A11Y_CSS = `
.rwa2-version,
.rwa2-context-chip,
.rwa2-token-trigger,
.rwa2-context-depth,
.rwa2-length-label,
.rwa2-length-toggle { font-size:11px !important; }

.rwa2-performance-key { font-size:10px; }
.rwa2-length-value { font-size:10px; }
.rwa2-performance-meta { font-size:10px; }
.rwa2-profile-name { font-size:10.5px; }
.rwa2-identity-chip { font-size:11.5px; }
.rwa2-brand-title { font-size:14.5px; }
.rwa2-popup .rwa2-action { font-size:10.5px !important; }
.rwa2-multi-notice { font-size:11px; }
.rwa2-tooltip { font-size:11px; line-height:1.45; }

.rwa2-popup :is(
  .rwa2-icon-button,
  .rwa2-trim-button,
  .rwa2-profile-btn,
  .rwa2-performance-item,
  .rwa2-context-chip,
  .rwa2-token-trigger,
  .rwa2-action,
  .rwa2-depth-step,
  .rwa2-length-toggle
):focus-visible {
  outline:2px solid rgba(226,161,59,.72);
  outline-offset:2px;
}

.rwa2-range:focus-visible {
  outline:2px solid rgba(226,161,59,.72);
  outline-offset:2px;
  border-radius:999px;
}

@media (prefers-contrast:more) {
  .rwa2-popup {
    --rwa2-border:rgba(255,255,255,.18);
    --rwa2-border-strong:rgba(255,255,255,.28);
    --rwa2-muted:rgba(255,255,255,.68);
    --rwa2-subtle:rgba(255,255,255,.54);
  }
}

`;
