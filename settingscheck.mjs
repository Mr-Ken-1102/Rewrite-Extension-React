...
assert.match(css, /\.rwas-settings\s*\{[\s\S]*font-family:\s*var\(--rwa-host-font, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif\)/);
...
assert.match(api, /marinaraTimeoutMs/);
assert.match(api, /Set timeout to 0 to wait until the provider finishes/);
assert.match(main, /--rwa-host-font/);
