...
- Moved persistent Character / Persona / Lore / Around defaults into Settings → Context so the popup drawer contains only per-request Adjust controls.
- Moved the active Character/Persona Voice Profile out of the header and into the spare final preset-grid slot; the built-in preset order stays unchanged and the profile remains keyboard/tooltip accessible.
- Request-summary chips now wrap instead of clipping, default Length Auto no longer consumes a chip, and popup placement reserves extra height when the summary needs a second row.
