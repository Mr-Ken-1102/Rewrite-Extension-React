...
const popupPerformance = read('./src/components/popup/PerformanceStrip.jsx');
...
assert.match(context, /DEFAULT REWRITE SOURCES/);
assert.match(context, /NGUỒN VIẾT LẠI MẶC ĐỊNH/);
assert.match(context, /config\.injectChar/);
...
assert.match(popupPerformance, /Fast Rewrite speeds up rewriting and returns results sooner/);
assert.match(popupPerformance, /Viết lại nhanh giúp tăng tốc xử lý và trả kết quả sớm hơn/);
assert.match(popupPerformance, /function fastRewriteHelp\(mode, vi\)/);
assert.match(popupPerformance, /disabled:\s*!capabilities\.fastRewrite/);
assert.match(popupPerformance, /updateConfig\(\{ fastRewrite: config\.fastRewrite === false \}\)/);
assert.match(popupPerformance, /streamingHelp\(capabilities\.liveStreaming, vi\)/);
assert.match(popupPerformance, /updateConfig\(\{ liveStreaming: config\.liveStreaming === false \}\)/);
