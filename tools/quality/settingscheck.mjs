...
const popupLiveRail = read('./src/components/popup/LiveRail.jsx');
...
assert.match(popupLiveRail, /Fast Rewrite speeds up rewriting and returns results sooner/);
assert.match(popupLiveRail, /Viết lại nhanh giúp tăng tốc xử lý và trả kết quả sớm hơn/);
assert.match(popupLiveRail, /function fastRewriteHelp\(mode, vi\)/);
assert.match(popupLiveRail, /disabled:\s*!capabilities\.fastRewrite/);
assert.match(popupLiveRail, /updateConfig\(\{ fastRewrite: config\.fastRewrite === false \}\)/);
assert.match(popupLiveRail, /streamingHelp\(capabilities\.liveStreaming, vi\)/);
assert.match(popupLiveRail, /updateConfig\(\{ liveStreaming: config\.liveStreaming === false \}\)/);
